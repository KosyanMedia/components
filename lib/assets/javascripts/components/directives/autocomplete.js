  ...
  return {
    ...
    controller: ['$scope', '$element', '$timeout', '$attrs', function($scope, $element, $timeout, $attrs){
      var
        ...
        restore_previous_value_on_blur = 'true' === $attrs.restorePreviousValueOnBlur,
        is_searching = false,
      ...
      $scope.$watch('acModel', function(new_value, old_value){
        $timeout(function(){$scope.$apply('last_search=acModel')}, 0);
      })
      ...
      this.set_items = function(items_scope){
        items_scope.$watch('selected_item', function(selected_item){
          if(restore_previous_value_on_blur && is_empty(selected_item)) {
            if(!$scope.last_search) { return; }
            $scope.last_search = angular.copy($scope.last_search);
            $scope.acModel = $scope.last_search;
          } else {
        ...
        input
          ...
          .bind('blur', function(){
            $timeout(function(){
              ...
              $scope.$apply('hovered_index = -1');
              $scope.$apply(function(scope){ scope.reject_current_input() });
            }, 200);
          })
